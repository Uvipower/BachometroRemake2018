...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 10-08-2017 a las 16:35:55
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `consultarReportes` (IN `_id_usuario` INT)  BEGIN
    SELECT r.id_reporte, r.fecha, r.latitud, r.longitud, r.referencia, r.comentario, r.imagen, h.estatus, h.fecha FROM reportes r INNER JOIN reporte_historial h ON r.Id_reporte=h.Id_reporte WHERE r.Id_usuario=_id_usuario ORDER BY r.id_reporte DESC ;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarInformacionUsuario` (IN `_apellido_paterno` VARCHAR(50), IN `_apellido_materno` VARCHAR(50), IN `_nombres` VARCHAR(50), IN `_id_usuario_datos` INT)  BEGIN
    UPDATE usuario_datos SET Apellido_paterno=_apellido_paterno, Apellido_materno=_apellido_materno, Nombres=_nombres WHERE Id_usuario_datos=_id_usuario_datos;
END$$
...
(1, 'uvs', 'd033e22ae348aeb5660fc2140aec35850c4da997', 'uvs@gmail.com', 2, NULL, 1, NULL),
